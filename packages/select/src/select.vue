<template>
  <div class="vc-select"
       :class="[selectSize ? 'vc-select--' + selectSize : '']"
    ...
    <div class="vc-select__tags"
      ...
      <span v-if="collapseTags && selected.length">
        <vc-tag :closable="!selectDisabled"
                :size="collapseTagSize"
                :hit="selected[0].hitState"
                type="info"
                @close="deleteTag($event, selected[0])"
                disable-transitions>
          <span class="vc-select__tags-text">{{ selected[0].currentLabel }}</span>
        </vc-tag>
        <vc-tag v-if="selected.length > 1"
                :closable="false"
                :size="collapseTagSize"
                type="info"
                disable-transitions>
          <span class="vc-select__tags-text">+ {{ selected.length - 1 }}</span>
        </vc-tag>
      </span>
      <transition-group @after-leave="resetInputHeight"
                        v-if="!collapseTags">
        <vc-tag v-for="item in selected"
                ...
                disable-transitions>
          <span class="vc-select__tags-text">{{ item.currentLabel }}</span>
        </vc-tag>
      </transition-group>
    ...
    <vc-input ref="reference"
      ...
      <template slot="suffix">
        <i v-show="!showClose"
           :class="['vc-select__caret', 'vc-input__icon', 'vc-icon-' + iconClass]"></i>
        <i v-if="showClose"
           class="vc-select__caret vc-input__icon vc-icon-circle-close"
    ...
    <transition name="vc-zoom-in-top"
      ...
      <vc-select-menu ref="popper"
                      :append-to-body="popperAppendToBody"
                      v-show="visible && emptyText !== false">
        <vc-scrollbar tag="ul"
                      wrap-class="vc-select-dropdown__wrap"
                      view-class="vc-select-dropdown__list"
                      ref="scrollbar"
                      :class="{ 'is-empty': !allowCreate && query && filteredOptionsCount === 0 }"
                      v-show="options.length > 0 && !loading">
          <vc-option :value="query"
                     created
                     v-if="showNewOption">
          </vc-option>
          <slot></slot>
        ...
        <template v-if="emptyText && (!allowCreate || loading || (allowCreate && options.length === 0 ))">
          ...
          <p class="vc-select-dropdown__empty"
             v-else>
            {{ emptyText }}
          </p>
        </template>
...
export default {
      ...
      if (!val) {
        ...
        if (this.filterable) {
          ...
          if (this.multiple) {
            ...
            if (!this.remote) {
              this.broadcast('VcOption', 'queryChange', '');
              this.broadcast('VcOptionGroup', 'queryChange');
            }
            this.broadcast('VcInput', 'inputSelect');
          }
        }
      }
  ...
  methods: {
    ...
    handleQueryChange (val) {
      ...
      this.previousQuery = val;
      this.$nextTick(() => {
        if (this.visible) this.broadcast('VcSelectDropdown', 'updatePopper');
      });
      this.hoverIndex = -1;
      ...
      if (this.remote && typeof this.remoteMethod === 'function') {
        ...
        this.broadcast('VcOption', 'queryChange', val);
        this.broadcast('VcOptionGroup', 'queryChange');
      }
      if (this.defaultFirstOption && (this.filterable || this.remote) && this.filteredOptionsCount) {
        this.checkDefaultFirstOption();
